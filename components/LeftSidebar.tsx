...
export const LeftSidebar: React.FC<LeftSidebarProps> = ({ items, selectedId, onSelect, onUpload, onDelete }) => {
  ...
  const handleDelete = (e: React.MouseEvent, id: string) => {
    ...
    if(window.confirm('정말 이 파일을 목록에서 삭제하시겠습니까?')) {
        // Optimistically remove from checked items to prevent UI stale state
  ...
  const handleBulkDelete = () => {
    ...
    if (window.confirm(`선택한 ${checkedItems.size}개의 파일을 정말 삭제하시겠습니까?`)) {
        ...
        setCheckedItems(new Set()); // Clear selection immediately
    }
  };
  ...
  return (
    <aside className="w-full md:w-72 bg-surface border-b md:border-b-0 md:border-r border-border flex flex-col shrink-0 transition-all duration-300 z-20 order-first h-auto max-h-[40vh] md:h-full md:max-h-none shadow-[5px_0_20px_rgba(0,0,0,0.02)]">
        ...
        <div className="p-4 border-b border-border flex-shrink-0 bg-surface-subtle/50">
          ...
          <div className="flex flex-col gap-2">
            <button 
                ...
                className="w-full py-3 bg-primary hover:bg-primary-hover text-white rounded-xl shadow-lg shadow-primary/20 flex items-center justify-center gap-2 font-bold text-sm transition-all active:scale-95"
            >
              ...
              <div className="flex items-center gap-2">
                  <div 
                    ...
                    className={`size-4 rounded border flex items-center justify-center cursor-pointer transition-colors ${
                        ...
                        : 'bg-white border-border text-transparent hover:border-primary/50'
                    }`}
                  >
                    ...
                    items.map((item) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div 
                                    ...
                                    className={`absolute left-2 top-1/2 -translate-y-1/2 z-10 size-5 rounded-full border flex items-center justify-center transition-all shadow-sm
                                        ${isChecked 
                                            ...
                                            : 'bg-white border-border text-transparent opacity-0 group-hover:opacity-100 hover:border-primary'}`}
                                >
                                    <span className="material-symbols-outlined text-xs font-bold">check</span>
                                </div>
                                ...
                                ${item.status === 'done' ? 'bg-white border-success/20 text-success' : 
                                item.status === 'processing' ? 'bg-white border-primary/20 text-primary' : 
                                ...
                                >
                                    {item.mediaType === 'image' && item.previewUrl ? (
                                        // Changed object-cover to object-contain and added background to preserve aspect ratio
                                ...
                                {!isChecked && (
                                    <button 
                                        ...
                                        className={`flex items-center justify-center size-7 rounded-lg transition-all z-10 shrink-0
                                            ${item.id === selectedId 
                                                ? 'text-text-secondary hover:text-error hover:bg-white hover:shadow-sm opacity-100' 
